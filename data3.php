...
function stopIdToMysql($stopId,$routeTag) {
	...
	foreach ($xml->predictions->children() as $predic) {
		if (isset($predic->prediction[0]["minutes"])) {
			if ($predic->prediction[0]["branch"] == $routeTag) {

				$iquery = sprintf("INSERT INTO basic (stopId,minutes,branch, vehicle, dirTag,stamp) VALUES('%s','%s','%s','%s','%s',NOW())",
					$stopId,
					$predic->prediction[0]["minutes"],
					$predic->prediction[0]["branch"],
					$predic->prediction[0]["vehicle"],
					$predic->prediction[0]["dirTag"]);
				if (!$nodb) {
